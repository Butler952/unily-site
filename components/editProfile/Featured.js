...
const Featured = ({
  ...
  const handleSelect = (index) => {
    handleEditProfileChangeView('Edit feature', index)
  }

  const handleAdd = () => {
    handleEditProfileChangeView('Add feature')
  }
      ...
      { userContext &&
        ...
        <div onClick={() => handleSelect(index)} role="button" key={index} className={`${styles.job} d-flex flex-column align-items-start`} style={{gap:'16px'}}>
          <div className="d-flex flex-row justify-content-between w-100" style={{gap:'24px'}}>
            <div className="w-100">
              <p className="large text-dark-high font-weight-semibold mb-0">{feature.name}</p>
              {/* <a target="_blank" href={feature.company_linkedin_profile_url} className="text-decoration-none"> */}
              {/* <p className="large text-dark-med mb-0">{feature.company}</p> */}
              {/* </a> */}
              {/* {feature.description ? <p className="text-dark-med mb-0 mt-3">{feature.description}</p> : null} */}
            </div>
