...
const Testimonials = ({
  ...
  return (
    ...
    {/* <p>{
      ...
      userContext.profile.experiences.sort(sortTestimonials).map((job, index) => 
        <div key={index}>
          <p>{job.title}</p>
        </div>
      )
      ...
      { userContext &&
        ...
        // userContext.profile.testimonials.sort(sortTestimonials).map((testimonial, index) => 
        userContext.profile.testimonials.sort(sortTestimonials).map((testimonial, index) => {
        ...
        <div onClick={() => handleSelect(index)} role="button" key={index} className={`${styles.job} d-flex flex-column align-items-start`} style={{gap:'16px'}}>
          <div className="d-flex flex-row justify-content-between w-100" style={{gap:'24px'}}>
            <div className="w-100">
              <p className="large text-dark-high font-weight-semibold mb-0">{testimonial.name}</p>
              {/* <a target="_blank" href={testimonial.company_linkedin_profile_url} className="text-decoration-none"> */}
              {/* <p className="large text-dark-med mb-0">{testimonial.company}</p> */}
              {/* </a> */}
              ...
              {/* {testimonial.description ? <p className="text-dark-med mb-0 mt-3">{testimonial.description}</p> : null} */}
            </div>
