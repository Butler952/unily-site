...
const Template = () => {
  ...
  let templates = [
    ...
    },
    {
      ...
      'active': !templateChanged ? userContext?.template == 'bento' : templateSelection == 'bento',
      'premium': true
    },
    {
      "id": uuidv4(),
      ...
      'active': !templateChanged ? userContext?.template == 'staccato' : templateSelection == 'staccato',
      'premium': true
    },
    {
      "id": uuidv4(),
      ...
      'active': !templateChanged ? userContext?.template == 'metro' : templateSelection == 'metro',
      'premium': false
    },
    {
      "id": uuidv4(),
      ...
      'active': !templateChanged ? userContext?.template == 'metro_night' : templateSelection == 'metro_night',
      'premium': false
    },
    {
      "id": uuidv4(),
      ...
      'active': !templateChanged ? userContext?.template == 'document' : templateSelection == 'document',
      'premium': false
    },
  ]
          ...
          <div className="d-flex flex-column py-4 py-md-5 align-items-center" style={{gap: '16px'}}>
            {templates.map((template, index) => 
              <div key={index} role="button" onClick={() => changeTemplate(template)} className={`d-flex flex-column radius-3 p-3 w-100 ${pageStyles.planCard} ${template.active && pageStyles.active}`} style={{gap: '16px'}}>
                ...
                {/* <div className={`${template.string !== 'dark' ? 'bg-light-900' : 'bg-dark-900' } radius-2 border-1 border-solid border-dark-300`} style={{position: 'relative', paddingTop: '56.25%'}}>
                  <div className="d-flex flex-column align-items-center" style={{position: 'absolute', top: 0, left: 0, width: '100%', padding: '8%'}}>
                    <div className={`radius-5 ${template.string !== 'dark' ? 'bg-dark-400' : 'bg-light-500' }`} style={{width: '12%', paddingTop: '12%'}}></div>
                    <div className="d-flex flex-column align-items-center w-100" style={{marginTop: '4%'}}>
                      ...
                      <div className={`radius-4 ${template.string !== 'dark' ? 'bg-dark-400' : 'bg-light-500' }`} style={{width: '50%', paddingTop: '4%', marginTop: '2%'}}></div>
                      <div className={`radius-4 ${template.string !== 'dark' ? 'bg-dark-400' : 'bg-light-500' }`} style={{width: '30%', paddingTop: '4%', marginTop: '2%'}}></div>
                    </div>
                    <div className="d-flex flex-row justify-content-center w-100" style={{marginTop: '8%'}}>
                      ...
                      <div className={`radius-4 ${template.string !== 'dark' ? 'bg-dark-400' : 'bg-light-500' }`} style={{width: '16%', paddingTop: '4%'}}></div>
                    </div>
                  </div>
                </div> */}
      ...
      <div className="d-flex flex-column align-items-center justify-content-center position-fixed w-100 h-100" style={{top: 0, left:0, zIndex:3}}>
        ...
        { showBuildingOverlayContentTwo &&
          <div className={`d-flex flex-column align-items-center justify-content-center position-fixed w-100 h-100 ${pageStyles.overlayContentTwo}`} style={{top: 0, left:0, zIndex:2}}>
            <h4 className="text-light-high text-center mb-3">Welcome to your Vitaely website</h4>
          </div>
        }
      </div>
      }
